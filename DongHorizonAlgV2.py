...
This version uses Jeong's line inference method, proposed in https://link.springer.com/article/10.1007/s11045-018-0602-4#Ack1
"""
import cv2 as cv
import numpy as np
import os
...
class DongLV2:
    def __init__(self, SDGD_th=18):
        ...
        self.hl_slope = None  # slope of the final horizon line on the processed/resized image
        self.hl_intercept = None  # intercept of the final horizon line on the processed/resized image
    ...
    def get_horizon(self, img):
        ...
        if self.F_det:  # we check this flag again because it can be changed in self.outlier_hl_handler()
            ...
            self.img_edges = np.zeros(shape=self.img_with_filtered_segs.shape, dtype=np.uint8)
            self.Y = np.nan
            self.phi = np.nan
            self.latency = np.nan
    ...
    def custom_ransac(self):
        ...
        for self.ransac_sample_index in range(0, self.M):  # we don't do sampling because the number of segments is
            ...
            #     continue
            # if self.minimum_error > self.length_weighted_error:
            #     self.minimum_error = self.length_weighted_error
            #     self.hl_slope = self.k
            #     self.hl_intercept = self.b
            #
            #     self.hl_slope_res = self.hl_slope  # self.hl_slope * (45 / 64)
            #     self.hl_intercept_res = self.survived_segs_ys[self.ransac_sample_index] -\
            #                             self.hl_slope_res * self.survived_segs_xs[self.ransac_sample_index]
        # print("all errors: ", self.length_weighted_error_list)
        ...
        self.hl_slope = self.survived_segs_k[self.hl_index]
        self.hl_intercept = self.survived_segs_b[self.hl_index]
        return
    ...
    def step(self):
        self.x_out = np.zeros((0,))  # initialize coordinates of output edge pixels
        self.y_out = np.zeros((0,))  # initialize coordinates of output edge pixels
        self.N_f = self.survived_segs.shape[0]
        if self.N_f == 0:  # True if no segment survived the filter.
            self.F_det = False
            return
        self.xs_f, self.ys_f = self.survived_segs[:, 0], self.survived_segs[:, 1]
        self.xe_f, self.ye_f = self.survived_segs[:, 2], self.survived_segs[:, 3]

        self.survived_segs_len = np.uint16(np.subtract(self.survived_segs_len, 1))  # All length values must be integers to allow subsequent pr-
        # ocessing. Subtracting 1 allows subtraction of 1 for N_f iteration. See Eq1 in handwritten theory: STP, page 2
        # for a recall.
        self.u = np.arange(0, self.survived_segs_len[0])  # the first element of self.survived_segs_len is the length of the longest segment
        for self.Len_f_n, self.xs_f_n, self.ys_f_n, self.xe_f_n, self.ye_f_n in \
                zip(self.survived_segs_len, self.xs_f, self.ys_f, self.xe_f, self.ye_f):
    ...
    def hough_transform(self):
        if not self.F_det:  # True if the edge map contains edges. False otherwise (zero edges detected)
            return
        self.img_edges = np.zeros(shape=(512, 640), dtype=np.uint8)
        self.img_edges[self.y_out, self.x_out] = 255
        # **
        print(self.img_edges.dtype)
        cv.imwrite("edges.png", self.img_edges)
        self.hough_lines = cv.HoughLines(image=self.img_edges, rho=2, theta=pi*(1/180),
                                         threshold=2)
